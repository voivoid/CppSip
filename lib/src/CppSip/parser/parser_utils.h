...
bool phrase_parse( std::istream& input, const Parser& parser, const Skipper& skipper, Attr& attr )
{
  input >> std::noskipws;
  return phrase_parse( boost::spirit::istream_iterator( input ), {}, parser, skipper, attr );
}

template <typename Parser, typename Skipper, typename Attr>
bool phrase_parse( const std::string& line, const Parser& parser, const Skipper& skipper, Attr& attr )
{
...
bool phrase_parse( std::istream& input, const Parser& parser, const Skipper& skipper )
{
  input >> std::noskipws;
  return phrase_parse( boost::spirit::istream_iterator( input ), {}, parser, skipper );
}

template <typename Parser, typename Skipper>
bool phrase_parse( const std::string& line, const Parser& parser, const Skipper& skipper )
{
...
bool phrase_parse_emplace( Input&& input, const Parser& parser, const Skipper& skipper, Attrs&... attrs )
{
  ...
  bool result = parse( std::forward<Input>( input ), parser, skipper, fusion_attrs );
  if ( !result )
  {
    return false;
  }
