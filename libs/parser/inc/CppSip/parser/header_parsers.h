...
inline const auto Content_Length = bsx3::rule<struct _content_length, CppSip::Message::Headers::ContentLength>{} = (bsx3::lit("Content-Length") | 'i') > HCOLON > bsx3::uint64;

// discrete-type = "text" / "image" / "audio" / "video" / "application" / extension-token (!!!)
inline const auto discrete_type = bsx3::string("text") | bsx3::string("image") | bsx3::string("audio") | bsx3::string("video") | bsx3::string("application");
...
inline const auto message_header = bsx3::rule<struct _msg_header, CppSip::Message::Header>{} = ( Call_ID | Content_Length | CSeq | Max_Forwards ) > CRLF;
